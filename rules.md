...
* 1조 1항을 위배한 자는 경고 1점\~3점을 부여한다. \[ 표지판, 책 등 종류를 불문한다 ]&#x20;
...
* 1조 6항을 위배한 자는 관리자의 재량에 따라 뮤트 또는 경고를 부여한다.&#x20;
...
* 플레이에 영향을 주는 변조 프로그램과 모드, 핵 등을 금한다.&#x20;
...
* 형평성 문제를 발생시키는 모든 형태의 자동 및 반자동 농사는 제재 대상이다.

&#x20;\[ ex) 선인장과 울타리, 물을 이용한 자동 농사와 같이 레드스톤 회로를 사용하지 않더라도, 자동 농사의 형태가 되면 금한다. ]

### 『5. 몹공장 및 네더천장, 목축』

* 다량의 몹 스폰을 유도하여 아이템을 취하는 형태의 몹 공장은 제재 대상이며, 아래 조건을 충족할 시, 몹 공장으로 간주 된다.
* 평지화 등 몹 스폰 조건을 충족시켜, 스폰률을 극대화 시키는 건축물
* 레드스톤과 호퍼를 사용하여, 자동 및 반자동으로 몹을 죽이는 건축물
* 몹스폰 장소 주변을 개조하거나 블록, 물 들을 이용하여 자신이 데미지를 받을 수없는 위치에 있을 경우 5조 2항 : 지옥 천장의 기반암 평지를 이용하여, 구조물을 건설하는 행위를 금한다. 5조 3항 : 동물 및 주민의 수는 개인과 마을 모두 각각 \[ 청크 당 5개체 ] 가 허용된다.
* 주민 또는 동물이 밀집되어 있을 경우, 삭제 처리 될 수 있다.
* 또는 관리진이 보기에 부적절하다고 판단되면 위와 같이 삭제 처리 될 수 있다. 5조 4항 : 몬스터를 잡을 시에는 몬스터가 자신을 공격 할 수 있어야 한다. 5조 5항 : 몹스폰 장소 주변을 개조하거나 블록, 물 들을 이용하여 자신이 데미지를 받을 수 없는 위치에 있을 경우 몹 공장 판정을 받는다.

### 『6. 도시 / 야생월드』

* 6조 1항 : 도시 및 야생월드에 방화, 과한 그리핑 (자신 아래 블록을 쌓아 빠르게 이동을 하는 기술을 뜻한다.) 악의적인 수직굴과 수직탑, 과한 부동산을 점유(땅따먹기)는 서버에서 금한다.
* 6조 2항 : 엔더포탈은 공공기물이며 부동산으로 점령 할 수 없다.
* &#x20;\[※부동산 강제 철거※] 6조 3항 : 개인의 부동산을 둘러 쌓아 타인의 영토 확장을 막는 행위를 금한다.
...
* 운영진에게 처벌 강요를 금한다. \[ 운영자는 수많은 업무를 진행하므로, 모든 채팅을 모니터링 할 수 없음.] \[ 일부 누락 된 처벌은 디스코드 <#931519465441591418> 게시판을 이용한다. ]&#x20;
